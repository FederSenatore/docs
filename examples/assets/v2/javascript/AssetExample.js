...
// Function used to print created asset by account and assetid
const printCreatedAsset = async function (algodclient, account, assetid) {
    // NOTE: if you have an indexer instance available, it is easier to use just:
    //     let accountInfo = await indexerClient.searchAccounts()
    //    .assetID(assetIndex).do();
    // then, in the loop below, use this to extract the asset for a particular account
...
// Function used to print asset holding by account and assetid
...
(async () => {
    ...
    // transaction.
    // In this example, we will account for changing transaction parameters
    ...
    params.flatFee = true;
    console.log(params);
    let note = undefined; // NOTE: arbitrary data to be stored in the transaction; here, none is stored.
    // Asset creation specific parameters:
    // The following parameters are asset specific.
    // Throughout the example, these will be re-used.
    // In this example, we will also change the manager later.
    let addr = recoveredAccount1.addr;
    // Whether user accounts will need to be unfrozen before transacting.
    let defaultFrozen = false;
    // Integer number of decimals for asset unit calculation.
    let decimals = 0;
    // Total number of assets of this type available for circulation.
    let totalIssuance = 1000;
    // Used to display asset units to user.
    let unitName = "LATINUM";
    // Friendly name of the asset.
    let assetName = "latinum";
    // Optional string pointing to an URL relating to the asset.
    ...
    // Specified address is considered as asset reserve
   ...
   // console.log("AssetID = " + assetID);
    ...
    // Change the manager by using an asset configuration transaction.

    // First update changing transaction parameters.
    // In this example, we will account for changing transaction
    // parameters before every transaction.
    ...
    // Opting in is required to transact with the new asset.
    // Accounts that want receive the new asset
    ...
    // The transaction must be signed by the account wishing to opt in to the asset.
    ...
    let xtxn = algosdk.makeAssetTransferTxnWithSuggestedParams(sender, recipient, closeRemainderTo, revocationTarget,
         amount,  note, assetID, params);
    // The transaction must be signed by the account sending the asset.
    ...
    // You should now see the frozen asset listed in the account information.
    console.log("Account 3 = " + recoveredAccount3.addr);
    await printAssetHolding(algodclient, recoveredAccount3.addr, assetID);


    // With the frozen vales set to true,
    // you should see a console/terminal output similar to this:
    //
    ...
    // The recipient will also be the creator (in this case,
    // it is account3).
    // First update changing transaction parameters.
    // In this example, we will account for changing transaction parameters
    // before every transaction.
    ...
    let rtxn = algosdk.makeAssetTransferTxnWithSuggestedParams(sender, recipient, closeRemainderTo, revocationTarget,
       amount, note, assetID, params);
    // The transaction must be signed by the account that is the clawback address.
    ...
    // You should see a console/terminal output similar to below for account 3:
    //
    ...
    // Destroy and Asset:
    // All of the created assets should now be back in the creator's
    // account, so that we can delete the asset.
    ...
    // the asset creator can sign and issue "txn", in order to remove the asset from the ledger. 
    ...
    // account3 and account1 should no longer contain the asset, as it has been destroyed.
    ...
    // metadata still exist in account holdings for Account 3. 
    ...
    // This is necessary for technical reasons, since we currently can't have a single transaction touching, potentially,
    // thousands of accounts (i.e. all the holdings that would need to be deleted).
